...
def _location_sheet_column_headers():
	...
	location_sheet_column_headers.append("Warranty Status")
...
def _raid_info_sheet_column_headers():
	...
	raid_info_sheet_column_headers.append("Disks Per RAID Group")
...
def _capacity_trending_sheet_column_headers():
	...
	capacity_trending_sheet_column_headers.append("Growth Rate (TB/month)")
	capacity_trending_sheet_column_headers.append("Growth Rate (GB/day)")
...
for serial_number in serial_numbers_list:	
	...
	asup_received_date 	= current_page._asup_received_date(asup_sysconfigR_url_output)
	biz_key 			= current_page._biz_key(asup_sysconfigR_url_output)
	asup_id 			= current_page._asup_id(asup_sysconfigR_url_output)
	location 			= current_page._location(asup_sysconfigR_url_output)
	cluster_name 		= current_page._cluster_name(asup_sysconfigR_url_output)
	host_name 			= current_page._host_name(asup_sysconfigR_url_output)
	system_model		= current_page._system_model(asup_sysconfigR_url_output)
	os_version 			= current_page._system_version(asup_sysconfigR_url_output)
	serial_number 		= current_page._serial_number(asup_sysconfigR_url_output)
	...
	if cluster_name == None:
		cluster_name = "No_Cluster_{0}".format(no_cluster_count)
		no_cluster_count+=1
...
total = t1-t0
time_per_serial_num = total/(len(serial_numbers_list))

print "TOTAL TIME ELAPSED = " + str(total)
print "AVG TIME PER SERIAL NUM = " +  str(time_per_serial_num)